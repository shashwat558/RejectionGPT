...
export default function ChatSidebar() {
  ...
  return (
    <motion.div
      ...
      className="h-full flex flex-col border-r border-gray-200 bg-gray-50/50 relative z-20"
    >
      {/* Header */}
      <div className="flex items-center justify-between p-5 mb-2 border-b border-gray-200">
        <AnimatePresence mode="wait">
          {!isCollapsed && (
            ...
            >
              ...
              <span className="font-semibold text-black tracking-tight whitespace-nowrap">RejectionGPT</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <button
          ...
          className="p-2 ml-auto rounded-lg hover:bg-gray-200 text-gray-500 hover:text-black transition-colors"
        >
      ...
      <div className="px-5 my-6 space-y-4">
        <Link
          href="/analytics"
          className={`flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-black text-white font-medium hover:bg-gray-800 transition-all ${
        ...
        {!isCollapsed && (
          <div className="relative group">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-black text-sm placeholder-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all"
            />
          </div>
        )}
      </div>
        ...
        <AnimatePresence>
          {filteredChats.map((chat) => {
            ...
            >
              {!isCollapsed && (
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    <h3 className={`text-sm font-semibold truncate ${isActive ? "text-black" : "text-gray-700 group-hover:text-black"}`}>
              ...
              {isCollapsed && (
                 <div className={`w-2.5 h-2.5 rounded-full mt-1.5 ${isActive ? "bg-black" : "bg-gray-300 group-hover:bg-gray-400"}`} />
              )}

             {isCollapsed && (
                <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 bg-white text-black text-xs px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-lg border border-gray-200">
                  <p className="font-semibold mb-0.5">{chat.title}</p>
      ...
      <div className="p-4 border-t border-gray-200 bg-white">
        <div className={`flex items-center gap-3 ${isCollapsed ? "justify-center" : ""}`}>
          <div className="w-10 h-10 rounded-full bg-black flex items-center justify-center shadow-sm overflow-hidden shrink-0">
             <span className="text-white font-bold text-sm">S</span>
          </div>
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
               <div className="flex items-center gap-2">
                 <p className="text-sm font-semibold text-black truncate">Shashwat</p>
                 <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-black border border-gray-200">PRO</span>
               </div>
